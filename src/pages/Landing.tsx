    ...
    <div className="relative min-h-screen w-full bg-[#070E0D] text-white selection:bg-brand/30 overflow-x-hidden">
      ...
      >
        <div className="flex items-center gap-2 overflow-visible h-full">
          <LogoMark className="w-36 h-36 text-white -mt-[4px]" />
        </div>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <>
              <span className="text-sm text-white/50">{user?.username || user?.display_name || user?.email?.split("@")[0] || "User"}</span>
              <Link
                to="/chat"
          ...
          >
            ...
            {user ? (
              <div className="space-y-2 pt-2">
                <span className="block text-xs text-white/50">{user?.username || user?.display_name || user?.email?.split("@")[0] || "User"}</span>
