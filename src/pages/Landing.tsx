  ...
  return (
    <div className="relative min-h-screen w-full bg-[#070E0D] text-white selection:bg-brand/30 overflow-x-hidden">
      ...
      <motion.nav
        ...
        className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between"
      >
        <div className="flex items-center gap-2">
          <LogoMark className="w-44 h-44 text-white -my-5" />
        </div>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <>
              <span className="text-sm text-white/50">{user?.display_name || user?.email?.split("@")[0] || "User"}</span>
              <Link
                to="/chat"
          ...
          >
            ...
            {user ? (
              <div className="space-y-2 pt-2">
                <span className="block text-xs text-white/50">{user?.display_name || user?.email?.split("@")[0] || "User"}</span>
